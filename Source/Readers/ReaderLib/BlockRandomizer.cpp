...
void BlockRandomizer::Prefetch(ChunkIdType chunkId)
{
    ...
    if (m_shouldPrefetch && m_prefetchedChunk != chunkId && chunkId != CHUNKID_MAX)
    {
        ...
        m_prefetch = std::async(launch::async, [this, chunkId]() { return m_deserializer->GetChunk(chunkId); });
    }
}
