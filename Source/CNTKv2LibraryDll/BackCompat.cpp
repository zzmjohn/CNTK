...
namespace CNTK
{
    namespace Internal
    {
        ...
        Variable GetVariable(const ComputationNodeBasePtr& inputNode,
        ...
        {
            std::stack<ComputationNodeBasePtr> nodeProcessingStack;
            nodeProcessingStack.push(inputNode);

            while (!nodeProcessingStack.empty())
            {
                ComputationNodeBasePtr node = nodeProcessingStack.top();

                auto iter = nodeToVariableMap.find(node);
                if (iter != nodeToVariableMap.end())
                {
                    nodeProcessingStack.pop();
                    continue;
                }
                ...
                else
                {
                    ...
                    {
                        if (nodeToVariableMap.find(i) == nodeToVariableMap.end())
                        {
                            childrenProcessed = false;
                            nodeProcessingStack.push(i);
                        }
                    }
                    ...
                    {
                        inputVars[i] = nodeToVariableMap[node->Input(i)];
            ...
            return nodeToVariableMap[inputNode];
        }
